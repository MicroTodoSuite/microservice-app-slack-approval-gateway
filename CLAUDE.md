...
## Stack
- Node.js >=24, ES modules, no framework -- plain AWS SDK v3, `@octokit/app`, `@slack/web-api`, `groq-sdk`.
...
## Structure
- `src/github-webhook-handler/` -- receives GitHub webhooks (`pull_request` on `microservice-app-gitops`, `workflow_job` action `waiting` on the five service repos). `handle.mjs` is the pure, dependency-injected logic; `index.mjs` is the real Lambda entry point that wires AWS Secrets Manager, Slack, and Groq.
...
- `src/shared/` -- `verify-signatures.mjs` (GitHub HMAC + Slack HMAC/replay-window verification), `pr-body-sections.mjs` (parses the same `## Heading` convention `.github`'s `validate-pr.py` requires), `summarize.mjs` (Groq call, prompted with only the verbatim technical text as ground truth), `github-app.mjs` (installation-token minting), `slack-client.mjs` (posts/updates Slack messages), `load-secrets.mjs` (cached AWS Secrets Manager reads).
...
## Conventions
- `handle.mjs` files take a `deps` object and return `{ statusCode, body }`; `index.mjs` files are the only place that touch `process.env`, AWS Secrets Manager, or the real Slack/GitHub/Groq clients. Keep that split -- it is what makes the handlers testable without a live Lambda.
- The Groq prompt in `summarize.mjs` must only ever be given verbatim, already-validated technical text (a PR's own required body sections, or deterministic job/run metadata) as its factual input. Never let it fetch or infer additional facts -- its output feeds a production approval decision.
...
- `npm test` runs entirely against fake dependencies; it never calls a real GitHub, Slack, or Groq API, and never touches AWS. There is no live end-to-end test in this repo -- that verification happens once the GitHub App exists, the Lambdas are deployed, and one real gitops PR / one real prod deployment triggers the flow for real.
